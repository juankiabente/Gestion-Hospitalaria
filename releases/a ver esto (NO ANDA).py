...
class Database:
    ...
    def registrar_usuario(self, usuario, contraseña, tipo='paciente'):
        try:
            self.cursor.execute('INSERT INTO usuarios (usuario, contraseña, tipo) VALUES (?, ?, ?)', 
    ...
    def validar_usuario(self, usuario, contraseña):
        self.cursor.execute('SELECT id, tipo FROM usuarios WHERE usuario=? AND contraseña=?', 
                          (usuario, contraseña))
        return self.cursor.fetchone()

    def agregar_turno(self, usuario_id, fecha, especialidad):
        self.cursor.execute('INSERT INTO turnos (usuario_id, fecha, especialidad) VALUES (?, ?, ?)',
                           (usuario_id, fecha, especialidad))
        self.conn.commit()

    def obtener_turnos(self, usuario_id):
        self.cursor.execute('SELECT fecha, especialidad, estado FROM turnos WHERE usuario_id=?', (usuario_id,))
        return self.cursor.fetchall()

    def agregar_diagnostico(self, usuario_id, diagnostico, observaciones=''):
        fecha = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        self.cursor.execute('INSERT INTO historias (usuario_id, fecha, diagnostico, observaciones) VALUES (?, ?, ?, ?)',
                           (usuario_id, fecha, diagnostico, observaciones))
        self.conn.commit()

    def obtener_historia(self, usuario_id):
        self.cursor.execute('SELECT fecha, diagnostico, observaciones FROM historias WHERE usuario_id=? ORDER BY fecha DESC', 
                          (usuario_id,))
        return self.cursor.fetchall()

    def generar_receta(self, usuario_id, contenido, medico):
        fecha = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        self.cursor.execute('INSERT INTO recetas (usuario_id, fecha, contenido, medico) VALUES (?, ?, ?, ?)',
...
class PanelPacienteWindow:
    def _init_(self, master, db, usuario_id, usuario):
        self.master = master
        self.db = db
    ...
    def ver_recetas(self):
        # Implementar la visualización de recetas
        messagebox.showinfo("Recetas", "Funcionalidad de ver recetas en desarrollo")
